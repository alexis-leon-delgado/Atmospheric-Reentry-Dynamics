%% PROGRAMA 1 - CAS DE REENTRADA BALÍSTICA
...
%     3. Es poden modificar els valors del coeficient balístic (beta) a tractar. Si es volen afegir més valors, afegir més components al vector "beta_vector"
...
grafiques_a_mostrar=[true;  % (1): Velocitat
                    true;   % (2): Desacceleració
                    true;   % (3): Pressió dinàmica
                    true;   % (4): Mach
                    true;   % (5): Reynolds
                    true;   % (6): Pressió estancament
                    true;   % (7): Entalpia estancament
                    true;   % (8): Flux de calor al punt d'estancament
                    true;   % (9): Temps d'entrada
                    true;   % (10): Abast
...
% Vector de betas avaluades
beta_vector=[100,500,1000,5000];
...
V_0=22500/unitsratio('feet', 'meter');
gamma_0=deg2rad(12);
range_0=0;
t_0=0;
...
beta=beta_vector(j)*unitsratio('feet', 'meter')^2*4.4482216152605;

% Declaració de les equacions diferencials
F_Vh=@(h,V,gamma,t,range,g,a,P) g*((0.5*( V/a )^2*gamma_aire*P )/beta-sin(gamma))/(V*sin(gamma));
...
V_punt=-Q./(beta_vector(j)*unitsratio('feet', 'meter')^2*4.4482216152605)+sin(gamma);
...
% Càlcul del flux de calor per la correlació de Sutton and Graves
...
plot(V,Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))])
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f1','velocitat_ballistic.png','-dpng','-r500');    
end
end

if grafiques_a_mostrar(2)==true
fig2=figure(2); % Desacceleració
set(fig2,'Renderer', 'painters', 'Position', [400 565 375 170]);
plot(-V_punt,Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))])
ylabel('Altitud $h\;\mathrm{(km)}$','Interpreter','latex')
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f2','desacceleracio_ballistic.png','-dpng','-r500');    
end
end

if grafiques_a_mostrar(3)==true
fig3=figure(3); % Pressió dinàmica
set(fig3,'Renderer', 'painters', 'Position', [785 565 375 170]);
plot(Q,Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))])
ylabel('Altitud $h\;\mathrm{(km)}$','Interpreter','latex')
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f3','pressio_dinamica_ballistic.png','-dpng','-r500');    
end
end
...
plot(M,Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))]);
ylabel('Altitud $h\;\mathrm{(km)}$','Interpreter','latex')
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f4','mach_ballistic.png','-dpng','-r500');    
end
end

if grafiques_a_mostrar(5)==true
fig5=figure(5); % Reynolds
set(fig5,'Renderer', 'painters', 'Position', [15 305 375 170]);
plot(Re,Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))])
ylabel('Altitud $h\;\mathrm{(km)}$','Interpreter','latex')
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f5','reynolds_ballistic.png','-dpng','-r500');    
end
end

if grafiques_a_mostrar(6)==true
fig6=figure(6); % Pressió estancament
set(fig6,'Renderer', 'painters', 'Position', [400 305 375 170]);
plot(P_t,Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))])
ylabel('Altitud $h\;\mathrm{(km)}$','Interpreter','latex')
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f6','pressio_estancament_ballistic.png','-dpng','-r500');    
end
end

if grafiques_a_mostrar(7)==true
fig7=figure(7); % Entalpia estancament
set(fig7,'Renderer', 'painters', 'Position', [785 305 375 170]);
plot(entalpia_estancament,Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))])
ylabel('Altitud $h\;\mathrm{(km)}$','Interpreter','latex')
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f7','entalpia_estancament_ballistic.png','-dpng','-r500');    
end
end

if grafiques_a_mostrar(8)==true
fig8=figure(8); % Calor segons Sutton Graves
set(fig8,'Renderer', 'painters', 'Position', [1170 305 375 170]);
plot(-q_SG,Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))])
ylabel('Altitud $h\;\mathrm{(km)}$','Interpreter','latex')
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f8','flux_calor_ballistic.png','-dpng','-r500');    
end
end

if grafiques_a_mostrar(9)==true
fig9=figure(9); % Temps d'entrada
set(fig9,'Renderer', 'painters', 'Position', [15 45 375 170]);
plot(t,Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))])
ylabel('Altitud $h\;\mathrm{(km)}$','Interpreter','latex')
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f9','temps_ballistic.png','-dpng','-r500');    
end
end

if grafiques_a_mostrar(10)==true
fig10=figure(10); % Abast (range)
set(fig10,'Renderer', 'painters', 'Position', [400 45 375 170]);
plot(range/1000,Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))])
ylabel('Altitud $h\;\mathrm{(km)}$','Interpreter','latex')
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f10','abast_ballistic.png','-dpng','-r500');    
end
end
...
plot(energia_dinamica,Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))])
ylabel('Altitud $h\;\mathrm{(km)}$','Interpreter','latex')
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f11','energia_dinamica_ballistic.png','-dpng','-r500');    
end
end
...
plot(rad2deg(gamma),Z/1000,'DisplayName',['$\beta=$ ', num2str(beta_vector(j))])
ylabel('Altitud $h\;\mathrm{(km)}$','Interpreter','latex')
...
hold on
if(j==length(beta_vector) && printear==true)
print('-f12','gamma_ballistic.png','-dpng','-r500');    
end
end
